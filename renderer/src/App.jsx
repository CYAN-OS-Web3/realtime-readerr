...
const AppShell = () => {
    ...
    // Also listen for auth sync messages from the main process (Electron)
    React.useEffect(() => {
        const unsub = ipcService.onAuthSync((data) => {
            if (data?.token) {
                localStorage.setItem('cyan_token', data.token);
                if (data.user) localStorage.setItem('cyan_user', JSON.stringify(data.user));
                setAuthUserId(data.userId || (data.user && (data.user.id || data.user.email)) || 'synced-user');
        ...
        <div className="relative h-screen bg-[#050505] text-gray-100 flex flex-col overflow-hidden font-sans selection:bg-cyan-500/30">
            ...
            {toast.show && (
                <div className="fixed bottom-6 right-6 z-[100] animate-slide-up">
                    <div className={`bg-gray-900/90 backdrop-blur-xl border rounded-2xl p-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-start gap-4 min-w-[320px] ring-1 transition-all duration-500 ${
